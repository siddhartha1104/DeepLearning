...
class InputHandle:
    ...
    def load(self):
        ...
        if self.num_paths == 3:
            ...
            self.data['input_raw_data'] = np.concatenate(
                (dat_1['input_raw_data'], dat_2['input_raw_data']), axis=0)
            dat_3 = np.load(self.paths[2])
            num_clips_2 = dat_2['clips'].shape[1]
            dat_3['clips'][:, :, 0] += num_clips_2
            self.data['clips'] = np.concatenate(
                (self.data['clips'], dat_3['clips']), axis=1)
            self.data['input_raw_data'] = np.concatenate(
                (self.data['input_raw_data'], dat_3['input_raw_data']), axis=0)
            #self.data['output_raw_data'] = np.concatenate(
            #    (dat_1['output_raw_data'], dat_2['output_raw_data']), axis=0)
