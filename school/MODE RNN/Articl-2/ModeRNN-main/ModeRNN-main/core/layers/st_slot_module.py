...
from core.layers.ConvLSTMCell import ModeCell
import numpy as np
...
class ModeRNNCell(nn.Module):
    ...
    def forward(self, x, prev=None, h=None, index=1):
        head = self.heads
        b = x.size(0)
        n = x.size(1)
        ...
        #prev_out = rearrange(slots_prev, 'b (h d) l w -> b h d l w', l=self.img_size, w=self.img_size, h=head)
