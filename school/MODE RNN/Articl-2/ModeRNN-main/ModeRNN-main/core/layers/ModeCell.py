...
from core.layers.ConvLSTMCell import ConvLSTMCell, MixConvLSTMCell, DynamicConvOrigin, ModeCell
...
class ConvLSTMAttention(nn.Module):
    def __init__(self, dim, img_size, heads = 8, dim_head = 64, kernel_size=3, q_stride=1, k_stride=1, v_stride=1, dropout = 0.,
        ...
        self.convlstm = MixConvLSTMCell(dim_head, dim, dim, img_size, 5, heads-1)
    ...
    def forward(self, x, prev=None, h=None):
        head = self.heads
        b = x.size(0)
        n = x.size(1)
        ...
        slots_prev = slots
        #slots = rearrange(slots, 'b h d l w -> b h d (l w) ')

        #slots = slots.view((b, -1, x.size(-2), x.size(-1)))
        #print("slots inputs", slots.size())
        q = self.to_q(slots)
        q = rearrange(q, 'b (h d) l w -> b h (l w) d', h=head)
        #M_concat_k = torch.cat((q, k), 2)
        #M_concat_v = torch.cat((q, v), 2)
        #print("q, k", q.size(), k.size())
        #mk = torch.cat()
        ...
        # s = rearrange(attn, 'b (h d) l w -> b h (l w) d', h=h)
        ...
        #print(out.size(), slots_prev.size(), c.size())
        h, slots = self.convlstm(out, combined, slots_prev)
        #h = self.MoE(out, x)
        #slots_out = x + self.layernorm(h)
        #h = x + self.layernorm(h)
        #slots = slots_prev + self.conv(self.layernorm(slots))
...
class ModeRNNAttention(nn.Module):
    def __init__(self, dim, img_size, heads = 8, dim_head = 64, kernel_size=3, q_stride=1, k_stride=1, v_stride=1, dropout = 0.,
                 last_stage=False):
        ...
        self.convlstm = ModeCell(dim_head, dim, dim, img_size, 5, heads-1)
        #self.MoE = DynamicConvOrigin(dim*heads, dim, dim, WH=1,M=heads - 1,G=1,r=2)
        self.dim_head = dim_head
        self.conv = nn.Conv2d(dim_head, dim_head,
                               kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x, prev=None, h=None, m=None):
        head = self.heads
        b = x.size(0)
        n = x.size(1)
        #b, n, _, h = *x.shape, self.heads
        ...
        out = rearrange(out, 'b (h d) l w -> b h d l w', l=self.img_size, w=self.img_size, h=head)
        #print(out.size(), slots_prev.size(), c.size())
        h, slots, m = self.convlstm(out, combined, slots_prev, m)
        #h = self.MoE(out, x)
        #slots_out = x + self.layernorm(h)
        #h = x + self.layernorm(h)
        #slots = slots_prev + self.conv(self.layernorm(slots))
