...
from core.layers.action_modernn import ConvLSTMAttention
...
class RNN(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        self.conv_first = nn.Conv2d(self.patch_ch, num_hidden[num_layers - 1],
        ...
        for i in range(num_layers):
            ...
            cell_list.append(
                ConvLSTMAttention(num_hidden[-1], self.rnn_width, num_slots)
            )
            #cell_list.append(
            #    ConvLSTMAttention(in_channel, num_hidden[i], self.rnn_width,
    ...
    def forward(self, all_frames, mask_true, train=True):
        ...
        #print(input_frames.size(), input_actions.size(), mask_true.size())
        ...
        for t in range(self.configs.total_length - 1):
            ...
            #h_t[0], c_t[0] = self.cell_list[0](net, h_t[0], c_t[0], action)
            h_t[0], c_t[0] = self.cell_list[0](net, action, c_t[0], h_t[0], 1)

            for i in range(1, self.num_layers):
                #h_t[i], c_t[i], memory = self.cell_list[i](h_t[i - 1], h_t[i], c_t[i], memory, action)
                h_t[i], c_t[i] = self.cell_list[i](h_t[i - 1], action, c_t[i], h_t[i], i)
